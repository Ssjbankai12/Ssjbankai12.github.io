...
//
// Enhancement (CS 499 Milestone Three): the encryption key is no longer a single
// hardcoded string. A HashKeyStore now manages multiple named keys with average
// O(1) lookup, so the program can hold several keys and select one by name. See
// KeyStore.h for the LinearKeyStore/HashKeyStore implementations and Benchmark.cpp
// for the performance comparison between the two.
...
std::string read_file(const std::string& filename)
{
    ...
    buffer << input_file.rdbuf();  // read file into buffer
...
// Builds the key store used by this program. In the original version, the
// key was a single hardcoded string. Here, multiple named keys are managed
// through a HashKeyStore, so a key can be looked up by name in average O(1)
// time regardless of how many keys are registered.
HashKeyStore build_key_store()
{
    HashKeyStore keyStore;
    keyStore.addKey("default", "password");
    keyStore.addKey("alt-key-1", "correcthorsebatterystaple");
    keyStore.addKey("alt-key-2", "trustno1");
    return keyStore;
}
...
int main()
{
  ...
  // Build the key store and look up the key to use by name instead of
  // relying on a single hardcoded key string.
  HashKeyStore keyStore = build_key_store();
  std::string key;
  if (!keyStore.findKey("default", key))
  {
      std::cerr << "Requested key was not found in the key store!" << std::endl;
      return 1;
  }
